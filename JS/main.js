...
let isLoading = false;
let favoritos = []; //<-se añade variable para guardar favporitos
let historial = []; 
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    showErrorMessage(
      "Error al cargar la tienda. Por favor, recarga la página."
    );
  }
});
...
function initDOMReferences() {
  DOM = {
    ...
    favoritosBtn: document.getElementById("favoritos"),
    historiall: document.getElementById("historial"),
  };
}

function AgregarFavoritos() {
    const existingItem = favoritos.find((item) => item.id === productId);
  
    if (existingItem) {
      console.log("el producto ya ha sido añadido a favoritos");
    } else {
      favoritos.push({
        id: producto.id,
  ...
  function botonfavo() {
    ...
    DOM.productosContainer.appendChild(favorite);
  }
...
async function fetchProducts() {
  ...
  try {
    ...
    productos = data.filter(isValidProduct).map(normalizeProduct);
...
function showRetryButton() {
  ...
  const errorText = createElement("div", {
    style:
      "color: var(--color-danger); font-size: 1.2rem; margin-bottom: 1rem;",
    textContent: "Error al cargar productos. Verifica tu conexión a internet.",
  });

  const retryBtn = createElement("button", {
    style:
      "margin-top: 1rem; padding: 0.5rem 1rem; background: var(--color-primary); color: white; border: none; border-radius: 0.5rem; cursor: pointer;",
...
function createProductCard(producto) {
  ...
  const description = createElement("p", {
    textContent:
      truncateText(producto.description, 80) || "Sin descripción disponible",
  });

  // Precio
  const price = createElement("p", {
    style:
      "font-weight: 600; color: var(--color-primary); font-size: 1.1rem; margin: 0.5rem 0;",
  ...
  const category = createElement("p", {
    style:
      "font-size: 0.8rem; color: var(--color-gray); text-transform: capitalize; margin-bottom: 1rem;",
...
function createCartItem(item) {
  const li = createElement("li", {
    style:
      "display: flex; flex-direction: column; gap: 0.5rem; border-bottom: 1px solid var(--color-gray-light); padding-bottom: 0.5rem;",
  });
  ...
  const titleDiv = createElement("div", {
    style:
      "font-weight: 600; font-size: 0.9rem; line-height: 1.2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;",
  ...
  const decreaseBtn = createElement("button", {
    style:
      "width: 24px; height: 24px; border: none; background: var(--color-gray-light); border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 0.8rem;",
    textContent: "-",
  });
  ...
  const removeBtn = createElement("button", {
    style:
      "padding: 0.25rem 0.5rem; border: none; background: var(--color-danger); color: white; border-radius: 4px; cursor: pointer; font-size: 0.8rem;",
    textContent: "🗑️",
  });
  // CAMBIO AQUÍ: Prevenir el evento de propagación
...
function initEventListeners() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      !DOM.panelCarro.contains(e.target) &&
...
function processPurchase() {
  ...
  setTimeout(() => {
    showNotification(
      `¡Compra realizada con éxito! Total: $${total.toFixed(
        2
      )} (${itemCount} productos)`,
      "success"
    );
...
function showErrorMessage(message) {
  ...
  const reloadBtn = createElement("button", {
    style:
      "padding: 0.75rem 1.5rem; background: var(--color-primary); color: white; border: none; border-radius: var(--radius-md); cursor: pointer; font-weight: 600;",
